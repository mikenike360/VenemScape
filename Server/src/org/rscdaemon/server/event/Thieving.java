...
public class Thieving {
	...
	public int ExpMultiplier = 1; // Modify this number to multiply the XP given.
	...
	private int [][] Doors = {
			{586, 581, 585, 581, 10, 13}, // Nat rune West house (In)
			...
			{538, 592, 538, 591, 8, 10}, // Some ardy door
			{538, 591, 538, 592, 8, 10} // some ardy door
	};

	//ID, LvlReq, Exp, RespawnTime, Loot then(amount)
	public int [][] Chests = {
			{340, 59, 250, 15000, 619, 2}, // Blood rune chest.
			{334, 18, 8, 10000, 10, 50}, // Next 2 nature chest.
			{336, 72, 500, 180000, 546, 1, 154, 1, 160, 1, 10, 1000}, // Good chest, ardy..	
								...
								world.getDelayedEventHandler().add(new MiniEvent(player, 1200){
									public void action(){
										...
										for(int i=4; i < Chests[ourChest].length - 1; i++) {
											InvItem loot = new InvItem(Chests[ourChest][i], Chests[ourChest][i + 1]);
											owner.getInventory().add(loot);
											i = i + 1;
										}
										if(ourChest == 340) owner.teleport(613, 568, true);
			...
			for(int i=0; i < Stalls.length; i++) {
				if(object.getID() == Stalls[i][1]) {
					...
					if (player.getMaxStat(17) < Stalls[i][0]) {
						player.getActionSender().sendMessage("Sorry, you need a thieving level of " + Stalls[i][0] + " to steal from that");
						player.setSpam(false);
						return;
					}
				}
			...
			world.getDelayedEventHandler().add(new ShortEvent(player) {
				public void action() {
					...
					if (!chanceFormulae(Stalls[curStall][0])) {
						...
						owner.setBusy(false);
						if (Rand(20) <= 3) {	
							if(StallNpcs[curStall][0] == -1) { } else {
								Npc person = world.getNpc(StallNpcs[curStall][0], StallNpcs[curStall][1], StallNpcs[curStall][2], StallNpcs[curStall][3], StallNpcs[curStall][4]);
								if(person != null) {
									owner.npcThief[curStall] = true;
									String str = caughtChats[Rand(caughtChats.length)];
									str = str.replace("-name-", owner.getUsername());
									owner.informOfNpcMessage(new ChatMessage(person, str, owner));								
									person.resetPath();	
								}
						...
						world.registerGameObject(new GameObject(object.getLocation(), 341, object.getDirection(), object.getType()));
						...
						owner.getActionSender().sendInventory();
						owner.incExp(17, Stalls[curStall][2] * ExpMultiplier, true, true);
						owner.getActionSender().sendStat(17);
			...
			world.getDelayedEventHandler().add(new ShortEvent(player) {
				public void action() {
					if(!chanceFormulae(Doors[curDoor][4])) {
						owner.getActionSender().sendMessage("You failed to unlock the door");
						owner.setSpam(false);
						owner.setBusy(false);
						return;
					} else {
						owner.getActionSender().sendMessage("You sucessfully unlocked the " + object.getDoorDef().name);
						doDoor();
						owner.incExp(17, Doors[curDoor][5] * ExpMultiplier, true, true);
						owner.getActionSender().sendStat(17);
						owner.setSpam(false);
						owner.setBusy(false);
						owner.teleport(Doors[curDoor][2], Doors[curDoor][3], false);
	...
	public static boolean thievingSuccess(int lvl, int reqLevel)
	{
		...
		double success = ((3.27 * Math.pow(10, -6)) * Math.pow(dif, 4) + (-5.516 * Math.pow(10, -4)) * Math.pow(dif, 3) + 0.014307 * Math.pow(dif, 2) + 1.65560813 * dif + 18.2095966) / 100.0;
		if(success < 0.35)
			success = 0.35;
		if(reqLevel < 15)
		{
				...
				public void arrived(){
					if(owner.getSpam()) {
						...
						if(owner.getCurStat(17) < lvl){
							owner.getActionSender().sendMessage("You must be at least "+lvl+" thieving to pick the "+affectedNpc.getDef().name+"'s pocket.");				owner.setBusy(false);
							//affectedNpc.unblock();
							owner.setBusy(false);
							owner.setSpam(false);
							return;
						...
						world.getDelayedEventHandler().add(new ShortEvent(owner){
							public void action(){
								...
								if(chanceFormulae(lvl)) {
									owner.setSpam(false);
									owner.getActionSender().sendMessage("You sucessfully stole from the "+affectedNpc.getDef().name);
									for(int i=0; i < Loot.length; i++) {
										owner.getInventory().add(new InvItem(Loot[i], Amount[i]));
									}								
									owner.getActionSender().sendInventory();
									owner.incExp(17, exp * ExpMultiplier, true, true);
									owner.getActionSender().sendStat(17);
									owner.setBusy(false);
									affectedNpc.unblock();
								}else{
									...
									owner.setSpam(false);
									owner.getActionSender().sendMessage("You fail to pick the "+affectedNpc.getDef().name+"'s pocket.");
									int temp = Rand(10);
									if(temp >= 3) {
										owner.setBusy(false);
										affectedNpc.unblock();
